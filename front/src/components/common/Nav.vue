  ...
  <div style="height: 93.7vh;">
    <ul class="navBox"
      ...
      <li class="navPrimary"
        ...
        <span class="linka da_bg"
              :class="{active:menu.id == $route.meta.cid}"
              @click="MenuParent(index, menu.url)">
          <i :class="menu.icon" />
          {{ menu.description }}
        ...
        <ul class="navSecond"
          ...
          <li v-for="child in menu.children"
              @click="$router.push({name: child.url})">
            <span class="linka zi_font"
                  :class="{active:child.id == $route.meta.cid}">{{ child.description }}</span>
          </li>
        </ul>
      </li>
