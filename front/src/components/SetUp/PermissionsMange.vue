<template>
  <div>
    <div class="head">
      ...
      <el-upload class="head-title" :on-success="handleImportSuccess" :on-error="handleImportError" :headers="uploadHeaders"
        accept="application/vnd.openxmlformats-officedocument.wordprocessingml.document" :action="Importurl">
        <el-button size="small" type="primary">导入权限</el-button>
        <span slot="tip" class="el-upload__tip"> 只能的导入 xlsx 文件，且大小不超过4M </span>
      </el-upload>
      <div class="head-action">
        <el-input v-model="customFilters[0].vals" prefix-icon="el-icon-search" placeholder="搜索权限路由" class="class_input_width">
        </el-input>
    ...
    <div class="content">
      <data-tables-server :data="PermissionsData.ListData" :filters="customFilters" :total="PermissionsData.total" @query-change="getData"
