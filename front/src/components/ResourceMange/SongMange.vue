  ...
  <div>
    <div class="head">
      ...
      <div class="head-action">
        ...
        <el-button type="primary" @click="goSeed">新增歌曲</el-button>
      </div>
    </div>

    <div class="content">
      <data-tables-server :data="SongsData.ListData" :filters="customFilters" :total="SongsData.total" @query-change="getData"
...
export default {
  ...
  methods: {
    ...
    deletes (row) {
      this.$confirm('真的要删除此歌曲吗？', '删除', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(async () => {
        this.loading = true
        const data = await utils.deleteSongs(row.Id)
        if (data.data.status) {
          this.$message({
    ...
    async getData (queryInfo) {
      ...
      this.SongsData.queryData = {
        limit: limit,
        offset: offset,
        name: this.customFilters[0].vals
      }
      await this.getSongs(this.SongsData.queryData)
      this.loading = false
    },
    goSeed () {
      this.$router.push({
