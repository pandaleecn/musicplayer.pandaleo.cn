<template>
  <div>
    ...
    <div class="content">
      <data-tables-server :data="PlaylistsData.ListData" :filters="customFilters" :total="PlaylistsData.total" @query-change="getData"
        ref="multipleTable" v-loading="loading" :pagination-props="{ pageSizes: [5, 10, 20] }" :page-size="10">

        <el-table-column prop="Name" label="歌单名" key="Name" sortable="custom">
        </el-table-column>
...
export default {
  ...
  methods: {
    ...
    async getData (queryInfo) {
      ...
      this.PlaylistsData.queryData = {
        limit: limit,
        offset: offset,
        name: this.customFilters[0].vals
      }
      await this.getPlaylists(this.PlaylistsData.queryData)
      this.loading = false
    },
    goSeed () {
      this.$router.push({
