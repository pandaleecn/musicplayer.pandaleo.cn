    ...
    <div class="know-container" v-loading="loading">
      <div class="auditDaka">
        <el-form @submit.native.prevent class="margin-top-lg" :label-position="labelPosition" label-width="130px" :model="ruleForm"
          ...
          <el-form-item prop="Name" class="margin-left-lg margin-top must" label="歌词名">
            <el-input class="form-input-h" v-model="ruleForm.Name" placeholder="请输入" />
            <p class="form_p_g">请输入歌词名称！</p>
          </el-form-item>
  ...
  methods: {
    ...
    async getData () {
      if (this.$route.params.id) {
        this.loading = true
        const data = await utils.getLyricDetail(this.$route.params.id)
    ...
    async postLyric () {
      this.loading = true
      const data = await utils.postLyric(this.ruleForm)
      if (data.data.status) {
        this.$message({
          type: 'success',
    ...
    async putLyric () {
      this.loading = true
      const data = await utils.putLyric(this.ruleForm)
      if (data.data.status) {
        this.$message({
          type: 'success',
          message: data.data.msg
